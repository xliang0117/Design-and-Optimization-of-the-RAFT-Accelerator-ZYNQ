...
void export_corr(ADT32 *fm, int32_t addr, hls::vector<RDT, 32> rfm[3][50][50], short group)  //one group is correspondant to 96 output channels
{
    uint offset = addr + group*3*2304;
        ...
        for (short row = 1; row < 49; row++)
        {
            for (short col = 1; col < 49; col++)
            {
                ...
                for (short c = 0; c < 32; c++)
                {
                    ...
                    ap_int<9> qy = rfm[op][row][col][c] >> 10; // div 8 = >> 3, scale = 7, so >> (3+7)
                ...
                fm[offset + op*2304 + (row-1)*48 + col - 1] = out_data;
            }
        }   
    }
