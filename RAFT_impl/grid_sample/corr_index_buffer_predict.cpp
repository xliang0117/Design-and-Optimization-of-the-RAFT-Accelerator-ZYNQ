...
void grid_predict_UPDATE_LOAD(ADT32 *fm, FDT flow[2304], bool update[2304], hls::stream<bool> &update_fifo, hls::stream<ADT8> &cb_fifo, short layerNum)
{
    short dim_limit;
    int offset;
    switch(layerNum)
...
void grid_predict_UPDATE_EXPORT(bool des[2304], hls::stream<bool> &update_fifo, ADT16* corr_buffer, hls::stream<ADT8> &cb_fifo)
{
    for (CDT row = 0; row < 48; row++)
    {
        for (CDT col = 0; col < 48; col++)
...
void grid_predict_UPDATE(ADT32 *fm, FDT flow[2304], bool update[2304], bool des[2304], ADT16* corr_buffer, short layerNum)
{
    ...
    grid_predict_UPDATE_LOAD(fm, flow, update, update_fifo, cb_fifo, layerNum);
    grid_predict_UPDATE_EXPORT(des, update_fifo, corr_buffer, cb_fifo);
}
