...
void load_fm_tile(ADT32 *ifm_ddr, int32_t addr, hls::vector<ADT, 32> ifm[50][50], short rowx, short colx, short chx, short width, short height) //all ex addr should before one row and one col
{
    int offset = addr + chx*width*height + rowx*48*width + colx*48 +width+1; //+width+1 for padding
...
void load_fm_s2(ADT32 *ifm_ddr, int32_t addr, hls::vector<ADT, 32> ifm[50][50], short rowx, short colx, short chx, short width, short height) //all ex addr should before one row and one col
{
    int offset = addr + chx*width*height + rowx*96*width + colx*96;
...
void load_fm_full(ADT32 *ifm_ddr, int32_t addr, hls::vector<ADT, 32> ifm[50][50], short chx) //all ex addr should before one row and one col
{
    int offset = addr + chx*48*48 +48+1; //+48+1 for padding
...
{
    int offset = addr + (inx)*outPart*32;
...
void load_dwbbuf(WDT32 *weight_ddr, int32_t addr, BDT bbuf[32], short inx)
{
    ...
    {
        #pragma hls pipeline II=1
        WDT32 in_ddr_data = weight_ddr[addr+2*inx+iter];
        for (int c = 0; c < 16; c++)
        {
...
void load_pwbbuf(WDT32 *weight_ddr, int32_t addr, BDT bbuf[3][32], short outPart)
{
    ...
    for (int op = 0; op < outPart; op++)
    {
        ...
        {
            #pragma hls pipeline II=1
            WDT32 in_ddr_data = weight_ddr[addr+2*op+iter];
            for (int c = 0; c < 16; c++)
            {
...
{
    int offset = addr + chx*width*height + rowx*48*width + colx*48;
...
{
    int offset = addr + chx*width*height + rowx*24*width + colx*24;
    ...
    for (short ch = 0; ch < chx; ch++)
    {
        ...
        for (short row = 1; row < 49; row++)
        {
            for (short col = 1; col < 49; col++)
            {
                ...
                {
                    #pragma HLS unroll
                    ap_int<20> qy = ofm[ch][row][col][c] + bbuf[ch][c];
                ...
                ofm_ddr[addr + ch*48*48 + (row-1)*48 + col - 1] = out_data;
            }
        }
    }
}
...
void export_flow(hls::vector<ADT, 32> ofm[50][50], FDT flow[2304], bool update[2304])
{
    ...
    FLOW_ROW:for (short row = 1; row < 49; row++)
    {
        FLOW_COL:for (short col = 1; col < 49; col++)
        {
            ...
            update[(row-1)*48+col-1] = (temp_flow[0] == 0 && temp_flow[1] == 0)? false : true;
