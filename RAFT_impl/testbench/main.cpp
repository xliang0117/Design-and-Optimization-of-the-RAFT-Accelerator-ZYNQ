...
void load_fm(ADT32 *fm, const char* filename)
{
    ...
    file.close();
    // LOG_INFO("name:%s, idx:%d\n", filename, idx);
    // load fm the last number would change, so load should follow the order
}
...
void save_image(ADT32* fm, const char* filename)
{
    int idx = 0;
    std::ofstream file(filename, std::ios::out | std::ios::binary);
    if (!file){ std::cout << "error" << std::endl; return ;}
    for(int idx=0; idx < 1920*135; idx++){
        int8_t temp_pixel[32];
        for (int c = 0; c < 32; c++){
            temp_pixel[c] = fm[idx][c];
        }
        file.write((char*)&temp_pixel[0], 32);
    }
    file.close();
}
...
void tb_conv()
{
    ...
    ADT32* IMAGE_OUT = new ADT32[1920*135];
    ADT32* fm = new ADT32[420000 + 1000];
    ADT32* weight = new ADT32[6400];

    int fmap1_addr = 173185; 
    int fmap2_addr = 182401; 
    int Corr0_addr = 191617;    
    int Corr1_addr = 357505;
    int Corr2_addr = 398977;
    int Corr3_addr = 409345;
    int net_addr = 147841;
    int inp_addr = 154753;
    ...
    // load_fm(fm+net_addr, "../../binary/u-net.bin");
    // load_fm(fm+inp_addr, "../../binary/u-inp.bin");
    // load_fm(fm+fmap1_addr, "../../binary/fmap1.bin");
    
    LOG_INFO_S("start testing!");
    
    conv_hw(IMAGE1, IMAGE_OUT, fm, weight, (ADT*) (fm+385), false);
    conv_hw(IMAGE2, IMAGE_OUT, fm, weight, (ADT*) (fm+385), true);

    validate_out(fm, fmap1_addr, "../../binary/fmap1.bin", 48, 64, 0);
    validate_out(fm, fmap2_addr, "../../binary/fmap2.bin", 48, 64, 0); 
    ...
    // validate_out(fm, 51073, "../../binary/fh-p1.bin", 48, 96, 0);
    // validate_flow(flow, "../../binary/final-flow.bin", 48);
    save_image(IMAGE_OUT, "../../binary/flow_RGBA.bin");
