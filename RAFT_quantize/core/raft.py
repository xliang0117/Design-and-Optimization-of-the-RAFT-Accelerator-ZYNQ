...
from update import TensorLookUp
...
class RAFT(nn.Module):
    def __init__(self, args):
        ...
        self.fnet = SmallEncoder(output_dim=64, act=args.act, logType=args.logType)        
        self.cnet = SmallEncoder(output_dim=hdim+cdim, act=args.act)
        self.update_block = SmallUpdateBlock(self.args, hidden_dim=hdim)
        self.tanh = TensorLookUp(args.act, "r-tanh", args.logType, func=1)
    ...
    def forward(self, images, iters=1, flow_init=None, upsample=True, test_mode=False):
        ...
        with autocast(enabled=self.args.mixed_precision):   # mixed precision usage, like float16 mixed float32, for training
            ...
            Activation.ch_div32_save(fmap1, "fmap1")
            Activation.ch_div32_save(fmap2, "fmap2")
